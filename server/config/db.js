...
async function createMysqlTables() {
  ...
  try {
    ...
    // Auto-migrate profile_image column in users
    try {
      await mysqlPool.query(`ALTER TABLE users ADD COLUMN profile_image LONGTEXT`);
    } catch (imgErr) {}

    // Auto-migrate profile_image column in farmers
    try {
      await mysqlPool.query(`ALTER TABLE farmers ADD COLUMN profile_image LONGTEXT`);
    } catch (fImgErr) {}

    // Seed Admin login account with correct canonical password
    try {
      await mysqlPool.query(`
        INSERT INTO users (id, full_name, email, password, phone, role, district)
        VALUES ('u_admin_default', 'Admin Administrator', 'admin@gmail.com', 'admin@123', '+91 94470 00001', 'admin', 'Kerala')
    ...
    // Create admin_notifications table if not present
    try {
      await mysqlPool.query(`
        CREATE TABLE IF NOT EXISTS admin_notifications (
          ...
          sender_admin VARCHAR(100) DEFAULT 'Admin Administrator',
    ...
    // Create contact_messages table if not present
    try {
      await mysqlPool.query(`
...
async function seedSqliteTables() {
  const schema = `
    ...
    CREATE TABLE IF NOT EXISTS admin_notifications (
      ...
      target_value TEXT,
      sender_admin TEXT DEFAULT 'Admin Administrator',
      status TEXT DEFAULT 'active',
      created_at TEXT DEFAULT CURRENT_TIMESTAMP
    );
  ...
  return new Promise((resolve, reject) => {
    sqliteDb.exec(schema, (err) => {
      ...
      // Ensure password and profile_image columns exist in users & farmers
      sqliteDb.run('ALTER TABLE users ADD COLUMN password TEXT', () => {
        sqliteDb.run('ALTER TABLE users ADD COLUMN profile_image TEXT', () => {
          sqliteDb.run('ALTER TABLE farmers ADD COLUMN profile_image TEXT', () => {
            // Seed default Super Admin user into SQLite
            sqliteDb.run(
              `INSERT OR IGNORE INTO users (id, full_name, email, password, phone, role, district, created_at)
